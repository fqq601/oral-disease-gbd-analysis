setwd("/Users/fqq/Documents/10 BAPC_")
...
#----发病数据需要的年龄分层------
age1 <- c("70-74",
          "75-79","80-84","85-89","90-94","95+")   ###20个年龄组
...
# 标准年龄结构数据age_stand
age_stand <- read.csv("GBD2021-70以上人口权重.csv")


# 2. 把文件里的权重列直接变成向量 wstand
wstand <- age_stand$wstand          # 列名必须跟文件里完全一致

# 3. 验证
str(wstand)     # 应该是 numeric 向量，长度与你模型需要的年龄组数一致
sum(wstand)     # 应该 = 1
...
###读取发病数据
IBD_china <- read.csv('Final_combined_data.csv')
...
#筛选性别
GBD_both_population<- subset(GBD_population,
...
#读取预测数据
population_predicntion <- fread("IHME_POP_2017_2100_POP_REFERENCE_Y2020M05D01.csv")


#计算预测的both
both <- population_predicntion %>% 
  # 只保留男女
  filter(sex_id %in% 1:2) %>% 
  # 按维度汇总
  group_by(location_id, location_name, age_group_id, age_group_name,
           year_id, measure_id, measure_name, metric_id, metric_name,
           scenario, scenario_name) %>% 
  summarise(
    sex_id      = 3L,          # GBD 标准：3 = Both
    sex         = "Both",
    val         = sum(val),
    upper       = sum(upper),  # 如果想要 Bootstrap 区间请用 rbind 后 quantile
    lower       = sum(lower),
    .groups     = "drop"
  )

# 把原表和 Both 合并（可选）
GBD_population_prediction <- bind_rows(population_predicntion, both)
...
#####合并人口学数据1990-2036#####
GBD <- rbind(GBD_both_population, GBD_population_prediction)  
...
GBD_China_both$age_name<-factor(GBD_China_both$age_name, 
...
#长改宽
GBD_China_both_n <- dcast(data=GBD_China_both, 
...
#改行名
rownames(GBD_China_both_n) <- GBD_China_both_n$year


#删除第一列
GBD_China_both_n <- GBD_China_both_n[,-1]



#转为数值、数据框
GBD_China_both_n <- apply(GBD_China_both_n, 
...
IBD_pro <- matrix(data=NA,                             #  使用 matrix() 函数创建一个矩阵，参数 data = NA 表示矩阵中的所有初始值为 NA，即缺失值。这个矩阵相当于一个空白表格，等待填充数据。
                  ...
                  ncol=ncol(GBD_China_both_n)) %>%     #  ncol() 函数用于获取数据框 GBD_China_Male_n 的列数(20)。GBD_China_Male_n 的列数(20)应该对应不同的年龄组，这一步是确保新创建的矩阵 IBD_pro 的列数与 GBD_China_Male_n 相同。例如，如果 GBD_China_Male_n 有 20 个年龄组，那么 IBD_pro 也会有 20 列，每列将来会用来存储相应年龄组的预测数据。
...
#年份做行名
rownames(IBD_pro) <- seq(2022, 2041, 1)
...
# 将分解结果保存为CSV文件
write.csv(both_de, "ASDR-both.csv")
