...
#先处理发生人数数据#
###筛选需要的数据
IBD_in_female<- subset(IBD_china,
                       
                       
                       (IBD_china$age_name %in% age1) &
                         
                         
                         IBD_china$sex_name=="Female"&
                         ...
                         IBD_china$cause_name== 'Oral disorders' &
                         
                         IBD_china$measure_name=='DALYs (Disability-Adjusted Life Years)') #这些指标可以改




unique(IBD_in_female$age_name)



####年龄改为因子，并按升序排列
IBD_in_female$age_name <- factor(IBD_in_female$age_name, 
                                 
                                 levels = c("70-74", "75-79", "80-84", "85-89", 
                                            "90-94", "95+"))


# 提取数据Measure_name,age_name,year,val
IBD_in_female <- IBD_in_female[,c("measure_name", "age_name","year","val")]




#长转宽
IBD_in_female_n <- reshape2::dcast(data=IBD_in_female, #  reshape2::dcast()：将数据从长格式转换为宽格式，适用于以某个变量为行，另一个变量为列的情况。
...
#让year做行名
rownames(IBD_in_female_n) <- IBD_in_female_n$year


#删除第一列
IBD_in_female_n <- IBD_in_female_n[,-1]
...
####含义####
IBD_in_female_n <- apply(IBD_in_female_n,
...
#筛选性别
GBD_female_population<- subset(GBD_population,
                               
                               GBD_population$sex_name =="Female")
...
#筛选预测数据
GBD_female_prediction <- GBD_population_prediction %>%    
  dplyr::select(prediction_var_name) %>%           
  dplyr::filter(location_name %in% 'Global' &        
                  year_id %in% 2022:2041   &         
                  sex %in% "Female")                  


unique(GBD_female_prediction$age_group_name)



GBD_5year <- GBD_female_prediction %>% 
...
GBD_female_prediction <- GBD_female_prediction %>% 
...
names(GBD_female_prediction)[names(GBD_female_prediction) == 'age_group_name'] <- 'age_name'  
...
GBD_female_prediction$age_name<-gsub(" to ","-",GBD_female_prediction$age_name)


GBD_female_prediction$age_name<-gsub(" plus","+",GBD_female_prediction$age_name)



unique(GBD_female_prediction$age_name)



colnames(GBD_female_prediction)<-var_name
...
#####合并人口学数据1990-2036#####
GBD_female <- rbind(GBD_female_population, GBD_female_prediction)  
...
GBD_female$age_name<-factor(GBD_female$age_name, levels = c("70-74", "75-79", "80-84", "85-89", 
                                                            "90-94", "95+"))
...
GBD_China_female <- subset(GBD_female,
...
GBD_China_female$age_name<-factor(GBD_China_female$age_name, 
...
#长改宽
GBD_China_female_n <- dcast(data=GBD_China_female, 
...
#改行名
rownames(GBD_China_female_n) <- GBD_China_female_n$year


#删除第一列
GBD_China_female_n <- GBD_China_female_n[,-1]



#转为数值、数据框
GBD_China_female_n <- apply(GBD_China_female_n, 
...
# 补充没有发病人数数据的年份
IBD_pro_female <- matrix(data=NA,                             #  使用 matrix() 函数创建一个矩阵，参数 data = NA 表示矩阵中的所有初始值为 NA，即缺失值。这个矩阵相当于一个空白表格，等待填充数据。
                         
                         
                         nrow=2041-2021,                      #  指定矩阵的行数为 2036 - 2021，即 15 行。这意味着矩阵的每一行代表一个年份，从 2022 年到 2036 年。不是16行.
                         
                         ncol=ncol(GBD_China_female_n)) %>%     #  ncol() 函数用于获取数据框 GBD_China_Male_n 的列数(20)。GBD_China_Male_n 的列数(20)应该对应不同的年龄组，这一步是确保新创建的矩阵 IBD_pro 的列数与 GBD_China_Male_n 相同。例如，如果 GBD_China_Male_n 有 20 个年龄组，那么 IBD_pro 也会有 20 列，每列将来会用来存储相应年龄组的预测数据。
...
#年份做行名
rownames(IBD_pro_female) <- seq(2022, 2041, 1)



#年龄组做列名
colnames(IBD_pro_female) <- names(IBD_in_female_n)


#合并1990-2041年的发病数据框
IBD_pro_n_female <- rbind(IBD_in_female_n, IBD_pro_female)



IBD_pro_n_female <- apply(IBD_pro_n_female, 
...
IBD_input_female <- APCList(IBD_pro_n_female, 
...
IBD_bapc_result_female <- BAPC(IBD_input_female, 
                               
                               predict=list(npredict=20, retro=T), 
...
p2<-plotBAPC(IBD_bapc_result_female, 
...
#提取数据
both_de =data.frame(IBD_bapc_result_female@agestd.rate)
...
# 将分解结果保存为CSV文件
write.csv(both_de, "ASDR-female.csv")
